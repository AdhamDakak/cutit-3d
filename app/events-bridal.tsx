...
export default function EventsBridalScreen() {
  ...
  const handleSelectStylist = (staffId: string, gender: Gender) => {
    ...
    router.push(`/booking-flow?type=events-bridal&gender=${genderParam}&staffId=${staffId}` as Href)
  }
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#f7f5f1] dark:bg-zinc-950" edges={['top', 'bottom']}>
      <View className="flex-row items-center justify-between border-b border-stone-200 bg-white px-5 py-4 dark:border-zinc-800 dark:bg-zinc-900">
        ...
        <Text className="flex-1 text-center font-serif text-lg font-semibold text-stone-900 dark:text-white">{t('eventsBridal.title')}</Text>
        <View className="size-9" />
      </View>

      <ScrollView className="flex-1" contentContainerClassName="gap-5 px-5 pb-10 pt-6">
        <Text className="text-sm text-stone-500 dark:text-zinc-400">{t('eventsBridal.subtitle')}</Text>
