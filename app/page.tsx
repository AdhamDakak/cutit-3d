      ...
      <div className={`mx-auto min-h-screen max-w-2xl ${bgClass} pb-24 ${!darkMode ? 'shadow-[0_0_60px_rgba(62,48,35,0.06)]' : ''}`}>
        <header className={`sticky top-0 z-20 border-b ${headerBgClass} px-5 py-4 backdrop-blur-md`}>
          ...
          <div className="mt-5 flex items-center justify-between">
            ...
            <div className={`grid size-10 place-items-center rounded-full border ${darkMode ? 'border-zinc-700 bg-zinc-800' : 'border-stone-200 bg-white'} text-sm font-semibold ${textClass}`}>AN</div>
          </div>
        </header>
        ...
        <nav className={`fixed inset-x-0 bottom-0 z-30 mx-auto flex max-w-2xl items-center justify-around border-t ${darkMode ? 'border-zinc-800 bg-zinc-900/95' : 'border-stone-200 bg-white/95'} px-3 py-3 backdrop-blur-md`} aria-label="Main navigation">
          {[{ label: 'Explore', icon: Compass }, { label: 'Bookings', icon: Clock3 }, { label: 'Saved', icon: Heart }, { label: 'Profile', icon: UserRound }].map(({ label, icon: Icon }) => (
            <button
              key={label}
